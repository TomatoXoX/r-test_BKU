...
class OpenFASTTestCaseGUI:
    ...
    def create_test_config_section(self, parent):
        ...
        dist_combo = ttk.Combobox(frame, textvariable=self.distribution_var, 
                                  values=["uniform", "normal", "logarithmic", "latin_hypercube", "grid_search"], width=15)
    ...
    def resolve_file_path(self, base_dir, filename):
        ...
        # 3. Fallback: Try just the filename in the same directory.
    ...
    def discover_parameters(self):
        ...
        try:
            ...
            while files_to_scan:
                ...
                if not file_path or file_path in processed_paths or not file_path.exists():
                    continue
    ...
    def extract_parameters_from_file(self, file_path, file_type):
        ...
        for i, line in enumerate(lines):
            ...
            if match:
                ...
                if param_name.lower() in ['true', 'false', 'default', 'unused', 'none', 'end', 'echo'] or any(ext in value_str for ext in ['.dat', '.txt', '.csv']):
                    continue
                param_info = self.parse_parameter_value(value_str, param_name, line)
                if param_info:
                    parameters[param_name] = {
                        'line_number': i, 'original_value': param_info['value'],
                        'type': param_info['type'], 'description': line.split('!', 1)[-1].split('-', 1)[-1].strip(),
    ...
    def parse_parameter_value(self, value_str, param_name, description):
        ...
        if value_str.upper() in ['DEFAULT', '"DEFAULT"']: return None
        try:
    ...
    def show_parameter_selector(self):
        ...
        tree.heading('#0', text='Parameter'); tree.heading('Type', text='Type'); tree.heading('Value', text='Current Value')
        tree.heading('Unit', text='Unit'); tree.heading('Description', text='Description')
        tree.column('#0', width=200); tree.column('Type', width=80); tree.column('Value', width=100, anchor='e')
        tree.column('Unit', width=80); tree.column('Description', width=350)
        ...
        def search_params(*args):
            ...
            for item, file_type, param_name, desc in all_items:
                if search_term in param_name or search_term in desc or search_term in file_type:
                    ...
                    tree.reattach(parent, '', 'end'); tree.item(parent, open=True)
    ...
    def add_parameter_with_info(self, file_type, param_name, param_info):
        ...
        start_var = tk.DoubleVar(value=start_default)
        ttk.Label(row_frame, text="Start:").grid(row=0, column=1, padx=(10, 2))
        ttk.Entry(row_frame, textvariable=start_var, width=10).grid(row=0, column=2, padx=(0, 5))
        
        end_var = tk.DoubleVar(value=end_default)
        ttk.Label(row_frame, text="End:").grid(row=0, column=3, padx=5)
        ttk.Entry(row_frame, textvariable=end_var, width=10).grid(row=0, column=4, padx=5)
        
        steps_var = tk.IntVar(value=5)
        ttk.Label(row_frame, text="Steps:").grid(row=0, column=5, padx=5)
        ttk.Spinbox(row_frame, from_=1, to=100, textvariable=steps_var, width=5).grid(row=0, column=6, padx=5)
        
        info_text = f"[{param_info.get('unit', '')}] (Current: {current_val:.4g})" if isinstance(current_val, float) else f"[{param_info.get('unit', '')}] (Current: {current_val})"
        ttk.Label(row_frame, text=info_text, foreground='gray').grid(row=0, column=7, padx=5, sticky='w')
            
        remove_btn = ttk.Button(row_frame, text="Remove", command=lambda: self.remove_parameter(row_frame))
        remove_btn.grid(row=0, column=8, padx=10)
        
        row_frame.columnconfigure(7, weight=1)

        self.parameter_entries.append({
            'frame': row_frame, 'start_var': start_var, 'end_var': end_var, 'steps_var': steps_var,
    ...
    def generate_test_cases(self):
        ...
        try:
            ...
            num_cases = parameter_values.shape[1] # Update num_cases in case grid_search changed it
            test_summary = []
            ...
            if messagebox.askyesno("Success", f"Generated {num_cases} test cases.\nSwitch to 'Run Tests' tab?"):
                self.notebook.select(self.run_tab); self.output_dir.set(str(output_path)); self.load_test_cases()
        except Exception as e:
            self.log(f"Error: {str(e)}"); messagebox.showerror("Error", f"Failed to generate test cases: {str(e)}")
    ...
    def modify_parameter_in_file(self, case_dir, file_type, param_name, value, param_info):
        ...
        if not file_path.exists():
            self.log(f"Warning: File {file_path} not found for parameter {param_name}"); return
        ...
        if not modified:
            for i, line in enumerate(lines):
                if re.search(r'\s' + re.escape(param_name) + r'\s', ' ' + line) and not line.strip().startswith(('!', '#')):
    ...
    def generate_parameter_values(self, num_cases):
        ...
        parameter_values, num_params, dist = [], len(self.parameter_entries), self.distribution_var.get()

        if dist == "grid_search":
            self.log("Using Grid Search to generate all parameter combinations.")
            if not self.parameter_entries: return np.array([])
            individual_param_steps = []
            for entry in self.parameter_entries:
                start_val, end_val, steps = entry['start_var'].get(), entry['end_var'].get(), entry['steps_var'].get()
                values = np.array([start_val]) if steps == 1 else np.linspace(start_val, end_val, steps)
                individual_param_steps.append(values)
            
            combinations = list(itertools.product(*individual_param_steps))
            parameter_values = np.array(combinations).T
            if parameter_values.shape[1] != num_cases: self.num_cases.set(parameter_values.shape[1])
        ...
        try:
            ...
            if dist == "latin_hypercube":
                self.log("Warning: 'scipy' not installed. Falling back to uniform."); self.distribution_var.set("uniform")

        sample = qmc.LatinHypercube(d=num_params).sample(n=num_cases) if dist == "latin_hypercube" and scipy_available else np.random.rand(num_cases, num_params)

        for j, param_entry in enumerate(self.parameter_entries):
            min_val, max_val = param_entry['start_var'].get(), param_entry['end_var'].get()
            
            if dist == "latin_hypercube" and scipy_available: values = qmc.scale(sample, [min_val]*num_cases, [max_val]*num_cases)[:, j]
            elif dist == "uniform": values = np.random.uniform(min_val, max_val, num_cases)
            elif dist == "normal": values = np.clip(np.random.normal((min_val + max_val) / 2, (max_val - min_val) / 6, num_cases), min_val, max_val)
            elif dist == "logarithmic":
                if min_val <= 0 or max_val <= 0:
                    self.log("Warning: Logarithmic scale requires positive min/max. Falling back to uniform.")
                    values = np.random.uniform(min_val, max_val, num_cases)
                else: values = np.logspace(np.log10(min_val), np.log10(max_val), num_cases)[np.random.permutation(num_cases)]
            else: values = np.random.uniform(min_val, max_val, num_cases)
    ...
    def browse_fst_file(self):
        ...
        if filename:
            self.base_fst_path.set(filename); self.log("Selected FST file: " + filename)
            if messagebox.askyesno("Discover Parameters", "Discover parameters for this file now?"): self.discover_parameters()
    ...
    def load_test_cases(self):
        ...
        self.case_tree.delete(*self.case_tree.get_children()); self.test_cases = {}
    ...
    def run_selected_cases(self):
        ...
        if not self.openfast_exe.get() or not Path(self.openfast_exe.get()).exists():
            messagebox.showerror("Error", "Please select a valid OpenFAST executable."); return
        ...
        threading.Thread(target=self.run_manager_thread, daemon=True).start()
    ...
    def save_config(self):
        ...
        config = {
            'base_fst_path': self.base_fst_path.get(), 'output_dir': self.output_dir.get(),
            'num_cases': self.num_cases.get(), 'distribution': self.distribution_var.get(),
            'parameters': [{'file_type': p['file_type'], 'param_name': p['param_name'],
                            'start': p['start_var'].get(), 'end': p['end_var'].get(), 'steps': p['steps_var'].get()
    ...
    def load_config(self):
        ...
        try:
            ...
            self.num_cases.set(config.get('num_cases', 5)); self.distribution_var.set(config.get('distribution', 'uniform'))
            self.clear_parameters()
            if self.base_fst_path.get() and not self.discovered_parameters:
                self.log("Base FST found, running discovery..."); self.discover_parameters()
            ...
            for param_config in config.get('parameters', []):
                ...
                if file_type and param_name and file_type in self.discovered_parameters and param_name in self.discovered_parameters[file_type]:
                    ...
                    entry['start_var'].set(param_config.get('start', param_config.get('min', 0)))
                    entry['end_var'].set(param_config.get('end', param_config.get('max', 1)))
                    entry['steps_var'].set(param_config.get('steps', 5))
    ...
    def on_distribution_change(self, event=None):
        """Handle changes in the distribution type combobox."""
        is_grid_search = (self.distribution_var.get() == "grid_search")
        self.num_cases_spinbox.config(state='disabled' if is_grid_search else 'normal')
        if is_grid_search: self.update_total_cases()

    def update_total_cases(self, *args):
        """Update the total number of cases for grid search."""
        if self.distribution_var.get() == "grid_search":
            total_cases = 1 if self.parameter_entries else 0
            for entry in self.parameter_entries:
                try: total_cases *= entry['steps_var'].get()
                except (tk.TclError, ValueError): total_cases *= 1
...
def main():
    ...
    app.log("Welcome to the OpenFAST Test Case Generator!"); app.log("=" * 60)
    app.log("1. Select your main '.fst' file and 'Discover Parameters'.")
    app.log("2. Choose a 'Distribution Type'. Use 'grid_search' for combinatorial studies.")
    app.log("3. 'Add from Discovery' to choose parameters to vary.")
    app.log("4. Set ranges (or Start/End/Steps for grid search).")
    app.log("5. Click 'Generate Test Cases'."); app.log("6. Go to 'Run Tests' to execute simulations."); app.log("=" * 60)
    root.mainloop()
