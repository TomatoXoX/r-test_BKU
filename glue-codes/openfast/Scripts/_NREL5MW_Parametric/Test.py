...
fast_input_file = r'C:\Users\Admin\Documents\HK243\DACN_DHBK\Code\r-test\glue-codes\openfast\5MW_Land_Linear_Aero\5MW_Land_Linear_Aero.fst'

# --- Define the range of rotor speeds to analyze (in RPM)
rotor_speeds_rpm = np.linspace(1, 15, 20) # From 0.1 to 12.5 RPM, 15 steps
...
def generate_campbell_data():
    ...
    for i, rpm in enumerate(rotor_speeds_rpm):
        ...
        fst['LinTimes'] = 0 # Time to linearize (ensure it's in steady state)
        fst.write()

        # 2. Set the initial rotor speed in the ElastoDyn file
        ed = FASTInputFile(r'C:\Users\Admin\Documents\HK243\DACN_DHBK\Code\r-test\glue-codes\openfast\5MW_Land_Linear_Aero\NRELOffshrBsline5MW_Onshore_ElastoDyn.dat')
        ed['RotSpeed'] = rpm
        ed.write()
    ...
    try:
        # The `Freq` dataframe returned here contains the UNDAMPED NATURAL FREQUENCIES
        ...
        # The `plotCampbell` function uses the `Freq` dataframe, which contains the natural frequencies.
        fig, axes = plotCampbell(OP, Freq, Damp, sx='RotSpeed_[rpm]', UnMapped=UnMapped, ps=[1, num_blades, 2*num_blades])
        
        # --- MODIFICATION: Make the plot label more explicit ---
