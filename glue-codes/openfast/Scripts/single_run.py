...
ref_dir   = os.path.join(scriptDir, './../5MW_Land_Linear_Aero')  # Folder where the fast input files are located (will be copied)
main_file = '5MW_Land_Linear_Aero.fst'  # Main file in ref_dir, used as a template
# --- MODIFICATION: Make the working directory an absolute path ---
work_dir  = os.path.join(os.path.abspath(scriptDir), '_NREL5MW_Parametric')

# --- Make sure the work_dir exists before using it
...
# --- 1. Define Key Operating Points and Simulation Settings ---
...
end_ws     = 16.0
end_pitch  = 10.4
num_points_below_rated = 4
num_points_above_rated = 2

# --- 2. Generate Parameters for the "Below Rated" Region ---
ws_below = np.linspace(start_ws, rated_wind_speed, num_points_below_rated)
rpm_below = np.linspace(start_rpm, rated_rpm, num_points_below_rated)
pitch_below = np.zeros(num_points_below_rated)

# --- 3. Generate Parameters for the "Above Rated" Region ---
ws_above = np.linspace(rated_wind_speed + 2.0, end_ws, num_points_above_rated)
rpm_above = np.full(num_points_above_rated, rated_rpm)
pitch_above = np.linspace(8.7, end_pitch, num_points_above_rated)

# --- 4. Combine the Regions and Create Final Lists ---
WS    = np.concatenate((ws_below, ws_above)).tolist()
RPM   = np.concatenate((rpm_below, rpm_above)).tolist()
PITCH = np.concatenate((pitch_below, pitch_above)).tolist()
BaseDict = {'TMax': 10, 'DT': 0.01, 'DT_Out': 0.1,'OutFileFmt': 3}
...
if __name__=='__main__':
    ...
    for p in PARAMS:
        ...
        # This now joins an absolute path with a filename, creating a guaranteed absolute path
